...
export function useNutrition() {
    const { t } = useLanguage();
    const [feedingPlans, setFeedingPlans] = useState<FeedingPlan[]>(initialFeedingPlans);
    const [supplements, setSupplements] = useState<Supplement[]>(initialSupplements);
    const [waterSchedule, setWaterSchedule] = useState<WaterSchedule[]>(initialWaterSchedule);
