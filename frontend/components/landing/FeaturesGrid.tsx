            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {features.map((f, i) => (
                        <div
                            key={i}
                            className={`landing-glass p-8 rounded-2xl transition-all duration-300 group hover:border-primary-gold/40 hover:shadow-[0_0_30px_rgba(212,175,55,0.08)] relative ${f.badge ? "border-primary-gold/30 shadow-[0_0_25px_rgba(212,175,55,0.05)]" : ""
                                }`}
                        >
                            {f.badge && (
                                <div className={`absolute ${language === "ar" ? "-top-3 -left-3" : "-top-3 -right-3"} bg-primary-gold text-midnight text-[10px] font-bold px-3 py-1 rounded-full uppercase z-10`}>
                                    {t(f.badge)}
                                </div>
                            )}
                            ...
                            <h3 className="text-xl font-bold mb-3 text-slate-100">{t(f.titleKey)}</h3>
                                ...
                                <ul className="space-y-2.5">
                                    {f.bullets.map((b, j) => (
                                        <li key={j} className="flex items-center gap-2 text-sm text-slate-300">
                                            <span className="material-icons text-primary-gold text-base flex-shrink-0">check_circle</span>
                                            {t(b)}
                                        </li>
                                    ))}
                                </ul>
