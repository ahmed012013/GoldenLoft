...
import { AlertTriangle, Calendar, DollarSign, Edit, Pill, Plus, Search, Trash2, TrendingDown } from "lucide-react";
...
export function InventoryMedicationsTab({
    ...
    const stats = {
        total: items.length,
        expiringSoon: items.filter((i) => getComputedStatus(i) === "expiringSoon").length,
                        ...
                        {items.length === 0 ? (
                            ...
                            items.map((item) => (
                                <TableRow key={item.id}>
                                    ...
                                    <TableCell>{getStatusBadge(getComputedStatus(item))}</TableCell>
                                    <TableCell>
