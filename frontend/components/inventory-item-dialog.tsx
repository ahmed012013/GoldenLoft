        ...
        switch (category) {
            case "feed":
                return [
                    ...
                    { value: "supplement", label: t("feedTypeSupplement" as any) || "Supplement" },
                ];
            case "medications":
                return [
                    { value: "vaccine", label: t("medTypeVaccine" as any) || "Vaccine" },
                    { value: "antibiotic", label: t("medTypeAntibiotic" as any) || "Antibiotic" },
                    { value: "vitamin", label: t("medTypeVitamin" as any) || "Vitamin" },
                    { value: "antiparasitic", label: t("medTypeAntiparasitic" as any) || "Antiparasitic" },
    ...
    useEffect(() => {
        if (open) {
            form.reset({
                ...
                purchaseDate: defaultValues?.purchaseDate ? new Date(defaultValues.purchaseDate).toISOString().split('T')[0] : "",
            });
        }
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    <DialogTitle>
                        {mode === "add" ? t("addItem" as any) || "Add Item" : t("editItem" as any) || "Edit Item"}
                    </DialogTitle>
                    <DialogDescription>
                        {mode === "add"
                            ? (category === 'feed' ? "Add new feed to inventory" : category === 'medications' ? "Add new medication" : "Add new equipment")
                            : "Update item details"}
                    </DialogDescription>
                </DialogHeader>
                    ...
                    <form onSubmit={form.handleSubmit(handleSubmit)} className="grid gap-4 py-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        <FormLabel>{t("unitPrice" as any) || "Unit Price"}</FormLabel>
                                        <FormControl>
                                            <Input type="number" step="any" {...field} />
                                        </FormControl>
                                        <FormMessage />
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        <FormLabel>{t("minStock" as any) || "Minimum Stock"}</FormLabel>
                                        <FormControl>
                                            <Input type="number" step="any" {...field} />
                                        </FormControl>
                                        <FormMessage />
                            ...
                            {category !== 'equipment' && (
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            <FormLabel>{t("expiryDate" as any) || "Expiry Date"}</FormLabel>
                                            <FormControl>
                            ...
                            {category === 'equipment' && (
                                <>
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                <FormLabel>{language === "ar" ? "الحالة" : "Condition"}</FormLabel>
                                                <Select onValueChange={field.onChange} defaultValue={field.value} value={field.value}>
                                                    <FormControl>
                                                        <SelectTrigger>
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                <FormLabel>{t("purchaseDate" as any) || "Purchase Date"}</FormLabel>
                                                <FormControl>
                                                    <Input type="date" {...field} />
                                                </FormControl>
                                                <FormMessage />
                        ...
                        <div className="flex justify-end gap-2 mt-4">
                            ...
                            <Button type="submit">
                                {mode === "add" ? (t("addItem" as any) || "Add Item") : (t("saveChanges" as any) || "Save Changes")}
                            </Button>
                        </div>
                    </form>
                </Form>
